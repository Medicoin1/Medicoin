...
uint256 hashGenesisBlock("0x000001b324519adf47b28e668397f5bdda3bdedc5bb6f802acce5840d6b3b38f");

static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // PYY: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "PYY Signed Message:\n";
...
{
    // PYY: IsDust() detection disabled, allows any valid dust to be relayed.
...
// Launch specs
static const int64 nTargetTimespan = 3 * 60; // PYY: each block
static const int64 nTargetSpacing = 1 * 60; // PYY: 300 seconds
...
unsigned int static GetNextWorkRequired_V1(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // PYY: This fixes an issue where a 51% attack can change difficulty at will.
...
unsigned int static DarkGravityWave(const CBlockIndex* pindexLast, const CBlockHeader *pblock) {
    /* current difficulty formula, PYY - DarkGravity, written by Evan Duffield - evan@PYY.io */
    const CBlockIndex *BlockLastSolved = pindexLast;
    const CBlockIndex *BlockReading = pindexLast;
    const CBlockHeader *BlockCreating = pblock;
    BlockCreating = BlockCreating;
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // PYY: Special short-term limits to avoid 10,000 BDB lock limit:
...
{
    // PYY: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0xa2ae3daf54d6deaf96b0ea626e05bfe63a22fac6b356ba8e0c25b35f7545fa17");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
      ...
      //vMerkleTree: 3f66108684400d84e2ffb528360d534fa19ecaf47c7c4c2ca1d1352ec4a351ad
        ...
        assert(block.hashMerkleRoot == uint256("0xb4b790867f229a5bfe35f3338bad15a856fb180470242330787537e63449f277"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xa6, 0xd2, 0xc3, 0xf6 }; // PYY: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("PYYMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("PYYMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static PYYMiner(CWallet *pwallet)
{
    printf("PYYMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("PYY-miner");
    ...
    try { while (true) {
        ...
        printf("Running PYYMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&PYYMiner, pwallet));
}
